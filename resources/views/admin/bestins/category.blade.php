@if ($category->max_points_percentage)
  <div class="font-bold">{{ $category->max_points_percentage }} {{ $contest->scoring_system == 'average' ? '%' : 'points' }}</div>
@endif
    ...
    @foreach ($category->ranked_contestants as $contestant)
      <tr class="{{ $loop->first ? 'bg-green-100' : '' }}"
        ...
        @foreach ($category->judges as $judgeKey => $judge)
          ...
          <td class="px-2 py-1 text-center border">{{ round($judgeScore['points_percentage'] ?? 0, 4) }}</td>
        @endforeach
