        ...
        <tbody>
          ...
          @foreach ($contestants as $contestant)
            ...
            @foreach ($contest->judges as $key => $judge)
              <tr>
                @if (!$key)
                  <td class="p-1 text-center align-top border"
                    rowspan="{{ $contest->judges()->count() }}">Top {{ $top }}</td>
                  <td class="w-auto p-1 align-top border"
                ...
                @foreach ($categories as $category)
                  @php
                    ...
                    if ($categoryJudge && $categoryContestant) {
                        $score = $category
                            ->categoryScores()
                            ->where([['category_judge_id', '=', $categoryJudge->id], ['category_contestant_id', '=', $categoryContestant->id]])
                            ->first()
                            ->criteriaScores()
                            ->sum('score');
            ...
            @endforeach
            <tr>
              <td class="p-1 font-bold text-right border"
                colspan="3">Average:</td>
              @foreach ($categories as $category)
                <td class="p-1 font-bold text-center border">
                  @php
                    $categoryContestant = $category->categoryContestants
                        ->filter(function ($categoryContestant) use ($contestant) {
            ...
            <tr>
              <td class="p-1 border"
                colspan="{{ $categories->count() + 4 }}">&nbsp;</td>
            </tr>
          @endforeach
        </tbody>
      </table>

      <p class="text-2xl font-bold">Breakdown</p>
      @foreach ($categories as $category)
        <p class="my-4 text-xl font-bold border-t">{{ $category->name }} - {{ $category->percentage }}%</p>
        <table class="w-full">
          <thead>
            <tr>
              <th class="p-1 border"
            ...
            @foreach ($category->categoryContestants as $categoryContestant)
              ...
              @foreach ($category->categoryJudges()->orderBy('judge_id')->get() as $key => $categoryJudge)
                <tr>
                  @if (!$key)
                    <td class="p-1 text-center align-top border"
                      rowspan="{{ $category->categoryJudges()->count() }}">Top {{ $top }}</td>
                    <td class="w-auto p-1 align-top border"
                      rowspan="{{ $category->categoryJudges()->count() }}">
                  ...
                  <td class="p-1 font-medium text-center border">
                    {{ round(
                        ($categoryContestant->categoryScores()->where('category_judge_id', $categoryJudge->id)->first()->criteriaScores()->sum('score') /
                            $category->criterias()->sum('percentage')) *
                            $category->percentage,
                        4,
                    ) }}
                  </td>
