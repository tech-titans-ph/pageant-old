...
<div class="mx-auto pt-12 pb-12">
	<div class="border-b p-4 text-center leading-normal">
		<h2 class="text-lg font-bold">{{ $contestCategory->contest->name }}</h2>
		<p class="text-md font-semibold">{{ $contestCategory->category->name }}</p>
		<p class="italic font-thin text-sm">Judge: {{ auth()->user()->name }}</p>
	</div>
	<div class="md:rounded md:shadow-md mx-auto">
		<div class="md:flex justify-center md:justify-between border-t">
			<div class="w-full md:w-1/2">
				<img src="{{ asset('storage/' . $categoryContestant->contestant->picture) }}" class="w-full object-contain" alt="Contestant">
			</div>
			<div class="w-auto md:w-1/2 flex md:static p-4 justify-between items-center">
				<a href="/judge-score/{{ $previousContestant->pivot->id }}" class="border rounded-full flex justify-center items-center no-underline block h-12 w-12 hover:bg-gray-200">
					<svg class="feather feather-chevron-left sc-dnqmqq jxshSx" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
						<polyline points="15 18 9 12 15 6"></polyline>
					</svg>
				</a>
				<div class="flex items-center justify-center md:px-8">
					<div class="flex items-center justify-center bg-green-600 rounded-full text-white mr-4 font-bold w-12 h-12">#{{ $categoryContestant->contestant->number }}</div>
					<div>
						<div class="font-bold">{{ $categoryContestant->contestant->name }}</div>
						<div class="font-thin">{{ $categoryContestant->contestant->address }}</div>
					</div>
				</div>
				<a href="/judge-score/{{ $nextContestant->pivot->id }}" class="border rounded-full flex justify-center items-center no-underline block h-12 w-12 hover:bg-gray-200">
					<svg class="feather feather-chevron-right sc-dnqmqq jxshSx" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" data-reactid="276">
